...
async function main({ args = [], stdin = '', env = {} }) {
  ...
  const { options, operands, unknown } = parseArgs.parse(args, getOptions());
...
function getDescription() { return 'Output the last part of files'; }

function getOptions() {
  return [
    { key: 'lines', short_tag: 'n', long_tag: 'lines', spec: 'string', help: 'output the last NUM lines' },
    { key: 'bytes', short_tag: 'c', long_tag: 'bytes', spec: 'string', help: 'output the last NUM bytes' }
  ];
}
